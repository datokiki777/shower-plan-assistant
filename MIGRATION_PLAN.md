...
| `archived` (boolean) | `Job.status = archived ? "archived" : "active"`, `Job.archivedAt = archived ? createdAt : null`, `Job.statusBeforeArchive = null` always. **Rationale (corrected):** V1 already has a working active/archived lifecycle (`archived: false` = currently active in the app today, `archived: true` = archived). Migration must be zero-surprise: it maps that lifecycle directly (`false` → `"active"`, `true` → `"archived"`) rather than guessing a richer status like `"completed"`. Nothing about a non-archived V1 report implies "finished" — inventing that would misrepresent real V1 records. The richer `"planned"`/`"completed"` states remain available in V2's `JobStatus` for jobs created after migration, but are never assigned to migrated records. `statusBeforeArchive` is always `null` on import (V1 has no equivalent concept), including for records with `archived: true` — this is exactly the "legacy archived record with no remembered prior status" case, and it's why `JobRepository.restore()` falls back to `"active"` rather than guessing: a V1-imported archived job restored in V2 becomes `"active"`, never a silently-invented `"completed"`. See DATA_MODEL.md §2. |
